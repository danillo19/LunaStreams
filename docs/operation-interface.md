...
Этот документ описывает, каким должен быть интерфейс операции, если система должна:
- оставаться удобной для Go в текущем runtime
- в будущем поддерживать операции на других языках
- иметь стабильный контракт между runtime и impl-операцией

Ниже описана рекомендуемая модель для следующей версии runtime. Это не описание текущего точного кода, а целевой интерфейс, который стоит ввести, чтобы поддержать polyglot-operations.
...
Главная проблема в `map[string]any`: такой формат не является хорошим wire contract.
